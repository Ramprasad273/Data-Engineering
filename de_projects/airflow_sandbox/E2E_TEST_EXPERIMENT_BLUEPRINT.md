...
def main() -> None:
    ...
    cur.close()
    conn.close()
    print("✅ Seeded 10,000+ clinical records into PostgreSQL.")
...
if [ "$SEVERITY" != "CRITICAL" ]; then
    echo "❌ Assertion Failed: Expected risk_severity CRITICAL, got $SEVERITY"
    exit 1
fi
echo "✅ Assertion Passed: Risk severity correctly identified as CRITICAL."
...
if [ $EXIT_CODE -ne 1 ]; then
    echo "❌ Assertion Failed: Expected exit code 1 from blocking gate, got $EXIT_CODE"
    exit 1
fi
echo "✅ Assertion Passed: Parallax successfully blocked CI with exit code 1."
...
if [ ! -f "e2e_clinical_blast_radius.html" ]; then
    echo "❌ Assertion Failed: e2e_clinical_blast_radius.html was not generated."
    exit 1
fi
...
echo "✅ Assertion Passed: Interactive HTML report generated with verified SVG DAG and exposures."
...
echo "✅ Assertion Passed: Markdown PR comment cleanly generated with marker for in-place updates."

echo ""
echo "========================================================="
echo "🎉 ALL E2E ARCHITECTURAL TESTS PASSED SUCCESSFULLY!     "
